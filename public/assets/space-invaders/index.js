...
function endGame() {
  ...
  setTimeout(() => {
    ...
    // Define the calculateCredits function
    const calculateCredits = (score) => {
      // Define your formula here to calculate credits based on the score
      const credits = Math.floor(score / 100) // For example, 1 credit for every 10000 points
      return credits
    }

    // Function to handle user's final score and calculate credits
    const handleFinalScore = (score) => {
      console.log('handling final score')
      const credits = calculateCredits(score)
      document.querySelector(
        '#credits'
      ).innerHTML = `Credits Earned: ${credits}`
      ...
      if (currentCredits === null) {
        ...
        localStorage.setItem('credits', parseInt(currentCredits) + credits)
      }
    }

    // Call the handleFinalScore function with the final score
...
document.querySelector('#shopButton').addEventListener('click', () => {
  // Perform the action to go back to the homepage here
  // For example, you can use the `window.location` object to navigate to the homepage URL
