...
function noMoney() {
    let InsertScore = totalAmountWon.toFixed(2)
    let userid = localStorage.getItem("userid")
    let gameid = localStorage.getItem("gameid")

    createScore(userid, gameid, InsertScore);
    messageEl.textContent = "You don't have enough money!"
}

// Optional: Close the stats dropdown when clicking outside of it
window.addEventListener('click', function(event) {
    const statsDropdown = document.querySelector('.stats-dropdown');
    if (!statsDropdown.contains(event.target)) {
      const statsToggle = statsDropdown.querySelector('.stats-toggle');
