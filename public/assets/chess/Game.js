class Game {
	...
	changeTurn() {
		...
		const whiteUsername = localStorage.getItem('whiteUsername');
		const blackUsername = localStorage.getItem('blackUsername');
		console.log(whiteUsername);
		console.log(blackUsername);
	
		if (this.turn === 'white') {
			this.turn = 'black';
			this.turnSign.innerHTML = `${blackUsername}'s Turn`;
		} else {
			this.turn = 'white';
			this.turnSign.innerHTML = `${whiteUsername}'s Turn`;
		}
	}
	...
	checkmate(color) {
		...
		if (color.toLowerCase() == 'white') {
			var winner = localStorage.getItem('whiteUsername');
			var loser = localStorage.getItem('blackUsername');

			
			// elo handling
		const winnerEloChange = 10; // Elo change for the winner
		const loserEloChange = -10; // Elo change for the loser
		...
		fetch('/api/updateElo', {
		  ...
		  .then(data => {
			if (data.success) {
			  ...
			  console.error('Failed to update Winner Elo');
			}
		  })
		  .catch(error => {
			console.error('Error updating Winner Elo:', error);
		  });
		...
		fetch('/api/updateElo', {
		  ...
		  .then(data => {
			if (data.success) {
			  ...
			  console.error('Failed to update Loser Elo');
			}
		  })
		  .catch(error => {
			console.error('Error updating Loser Elo:', error);
		  });

		} else if (color.toLowerCase() == 'black') {
			var winner = localStorage.getItem('blackUsername');
			var loser = localStorage.getItem('whiteUsername');
