...
// Import the createScore function from the specified module
...
function createFood() {
  ...
  let foodX =
    Math.floor(Math.random() * (gameBoardWidth / unitSize)) * unitSize;
  let foodY =
    Math.floor(Math.random() * (gameBoardHeight / unitSize)) * unitSize;
...
function moveSnake() {
  const head = {
    x: snake[0].x + snakeXVelocity,
    y: snake[0].y + snakeYVelocity,
  };
